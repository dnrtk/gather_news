...
_SLOT_SHORT = {"morning": "朝", "evening": "夕", "weekly": "週刊"}
...
def _parse_entry(html_path: Path, project_root: Path) -> dict | None:
    ...
    label = f"{d.year}/{d.month}/{d.day}({_WEEKDAY_JA[d.weekday()]}) {_SLOT_SHORT.get(slot, slot)}"
    return {
        "date_str": date_str,
        "date": d,
        "slot": slot,
...
def _write_nav_json(entries: list[dict], project_root: Path) -> None:
    nav = [{"label": e["label"], "path": e["rel_path"], "is_weekly": e["is_weekly"]} for e in entries]
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar-title { padding: .85rem 1rem .85rem 3.5rem; font-weight: 700; font-size: .9rem; border-bottom: 1px solid var(--border); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    #nav-list { flex: 1; padding: .5rem 0; }
    .nav-link { display: block; padding: .35rem 1rem; font-size: .875rem; color: var(--fg); text-decoration: none; border-radius: 0 4px 4px 0; margin-right: .5rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    .nav-empty { padding: 1rem; font-size: .85rem; color: var(--fg-muted); }
    ...
    async function loadNav() {
      try {
        ...
        for (const page of data) {
          html += `<a href="${page.path}" class="nav-link">${page.label}</a>`;
        }
